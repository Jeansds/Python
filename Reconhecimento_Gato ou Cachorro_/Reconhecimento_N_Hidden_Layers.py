...
def nn_model(X_train, Y_train, X_test, Y_test, n_h, num_iterations = 1000, learning_rate = 0.05, print_cost=False):
    ...
    return parameters
'''
parameters = nn_model(train_set_x, train_set_y, test_set_x, test_set_y, n_h = 10, num_iterations = 1000, learning_rate = 0.05, print_cost=True)

test_image = "temperatura-cachorro.jpg"
...
'''
learning_rates=[0.005]#,0.03,0.01]
models = {}

for i in learning_rates:
    print("learning rate is: ",i)
    models[i] = nn_model(train_set_x, train_set_y, test_set_x, test_set_y, n_h = 30, num_iterations = 10000, learning_rate = i, print_cost=True)
    print("-----------")

for i in learning_rates:
    plt.plot(np.squeeze(models[i]["costs"]),label=str(i))
